...
def register():
    ...
    regist = requests.post(
        f'http://localhost:8000/registration/', json.dumps(data))
...
def propost():
    ...
    product = requests.post(
        f'http://localhost:8000/product/', json.dumps(data))
...
def specpro():
    ...
    if product.content == {"name": "1984", "genre": "fiction", "price": 20, "quantity": 10, "business_id": 1, } and product.status_code == 200:
        return True
    else:
        return False
...
def patchpro():
    ...
    product = requests.patch(
        f'http://localhost:8000/products/1', json.dumps(data))
    if product.status_code == 200:
        return True
    else:
        return False
...
def addfile():
    ...
    addfile = requests.post(
        url="http://localhost:8000/product/1", files=files)
