...
def verify():
    global token
    token = os.popen(
        f'curl -X "POST" "http://127.0.0.1:8000/token" -N -H "accept: application/json" -H "Content-Type: application/x-www-form-urlencoded" -d "grant_type=&username={username}&password={password}"').read()
    token = str(token)
    token = eval(token)
...
def specpro():
    ...
    return (p["data"]["product_details"]["name"] == "1984" and p["data"]["product_details"]["price"] == 20 and product.status_code == 200)
